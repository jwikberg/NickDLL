...
vtable* mar_second_vtable = new vtable((BYTE*)0x96CAF4, 0xB4);

void mar_second_free_under(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	data->comp_vtable = (DWORD*)(mar_second_vtable->vtable_ptr);
	sub_687970(_this, 0);
...
void mar_second_free(BYTE* _this, BYTE a2) {
	mar_second_free_under(_this);
	if (a2 & 1) {
		sub_944C94_free(_this);
	}
}

void __declspec(naked) mar_second_free_c()
{
	__asm
	{
		mov eax, esp
		...
		push ecx
		call mar_second_free
		add esp, 0x8
		ret 4
	}
}

void mar_second_subs(BYTE* _this)
{
	...
	comp_data->promotes_to = MAR_FIRST_9CF();
...
void __declspec(naked) mar_second_subs_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call mar_second_subs
		add esp, 0x4
		ret
	}
}

DWORD mar_second_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	comp_stats* data = (comp_stats*)_this;
	if (stage_idx == -1) {
		if (a5)
...
void __declspec(naked) mar_second_fixtures_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call mar_second_fixtures
		add esp, 0x14
		ret 0x10
	}
}

char mar_second_update(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	data->f76 = 0;
	sub_687970(_this, 0);
	if (data->fixtures_table) {
...
void __declspec(naked) mar_second_update_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call mar_second_update
		add esp, 0x4
		ret
	}
}

void mar_second_playoff_under(BYTE* _this) {
	char stage_num = 0;
	comp_stats* comp_data = (comp_stats*)_this;
	...
	vector<cm3_clubs*> available_clubs = find_clubs_of_comp(A_LOWER_9CF(), NATION_MOROCCO_9CF());
...
void mar_second_playoffs_c(BYTE* _this) {
	comp_stats* comp_data = (comp_stats*)_this;
	...
	if (current < max - 1) {
		...
		if (current == 0) {
			...
			mar_second_playoff_under(_this);
		}
	}
}

void __declspec(naked) mar_second_playoffs_create()
{
	__asm
	{
		mov eax, esp
		push ecx
		call mar_second_playoffs_c
		add esp, 0x4
		ret
	}
}

int mar_second_table_fates(BYTE* _this, cm3_clubs* club, BYTE fate, char stage, BYTE* a5, BYTE* round_data, int a7) {
	...
	if (stage == 0) {
		...
		cm3_club_comps* mar_first = get_comp(MAR_FIRST_9CF());
		switch (fate) {
		case Champions:
		...
		case Promoted:
			...
			return 0;
		case TopPlayoff:
			staff_history_qualified_86BDD0(staff_hist_ptr, club, (DWORD)mar_first, None, Playoff, 0x1E);
			return 0;
		case BottomPlayoff:
...
void __declspec(naked) mar_second_table_fates_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call mar_second_table_fates
		add esp, 0x1c
		ret 0x18
	}
}

void mar_second_reputation_calc(BYTE* _this, BYTE* club, char stage, char current, char min, char max) {
	comp_stats* comp_data = (comp_stats*)_this;
...
void __declspec(naked) mar_second_reputation_calc_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call mar_second_reputation_calc
		add esp, 0x18
		ret 0x14
	}
}

int mar_second_stage_news(BYTE* _this, int club_idx, char fate, char stage_id, int stage_name_idx, int round_data, __int16 a7, int a8, char a9, int show_body_text, LPVOID* ret_str_ptr) {
	comp_stats* data = (comp_stats*)_this;
...
void __declspec(naked) mar_second_stage_news_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call mar_second_stage_news
		add esp, 0x2c
		ret 0x28
	}
}

void mar_second_init(BYTE* _this, WORD year, cm3_club_comps* comp)
{
	sub_682200(_this);
	comp_stats* data = (comp_stats*)_this;
	data->competition_db = comp;
	data->comp_vtable = (DWORD*)(mar_second_vtable->vtable_ptr);
	mar_second_vtable->SetPointer(VTableEoSUpdate, (DWORD)&mar_second_update_c);
	mar_second_vtable->SetPointer(VTableSubsRounds, (DWORD)&mar_second_subs_c);
	mar_second_vtable->SetPointer(VTableFixtures, (DWORD)&mar_second_fixtures_c);
	mar_second_vtable->SetPointer(VTableInitFree, (DWORD)&mar_second_free_c);
	mar_second_vtable->SetPointer(VTableReputationCalc, (DWORD)&mar_second_reputation_calc_c);
	mar_second_vtable->SetPointer(VTablePlayoffQual, (DWORD)&mar_second_playoffs_create);
	mar_second_vtable->SetPointer(VTableTableFates, (DWORD)&mar_second_table_fates_c);
	mar_second_vtable->SetPointer(VTableStageNews, (DWORD)&mar_second_stage_news_c);
	if (configFile.GetBool("showThirdPlaceInHistory", true)) mar_second_vtable->SetPointer(VTableShowThirdInHistory, 0x4110b0);
	data->year = year;
